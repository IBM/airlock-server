...
public class InputSchema {
	...
	public static String validateRulesAndConfigWithNewSchemaOrChangedUtility (final Season season, JSONObject schemaJson, final ServletContext context,
																			  UUID removedUtilityId, String newUtilityCode, Stage newUtilStage,
																			  final LinkedList<JSONObject> brokenRules, final LinkedList<JSONObject> brokenConfigs,
																			  ...
																			  LinkedList<JSONObject> brokenNotificationRules, LinkedList<JSONObject> brokenNotificationConfigs) throws JSONException, GenerationException, InterruptedException, ExecutionException
	{
		...
		final ValidationCache tester = new ValidationCache(newUtilityCode, id, newUtilStage); // cache with changed utility
...
//		ExecutorService executor = Executors.newFixedThreadPool(numBranches);
		for (final Branch b:season.getBranches().getBranchesList()) {
	...
	private static List<Future<String>> doValidateExperimentsRulesWithNewSchema(final Season season, //JSONObject schemaJson,
																				final ValidationCache tester, final ServletContext context,
																				final LinkedList<JSONObject> brokenExperiments, final LinkedList<JSONObject> brokenVariants) throws JSONException, GenerationException {
		...
		for (final Experiment exp:seasonExperiments) {
	...
	public JSONObject generateInputSample(Stage stage, String minAppVersion, ServletContext context, InputSampleGenerationMode generationMode, double randomizer) throws GenerationException, JSONException {
		...
		String inputSampleStr = JsonGenerator.generation(generator, faker, prune, getMergedSchema().toString(), stage.toString(), minAppVersion, generationMode, randomizer); //TODO: is it the right randomizer?
	...
	public static JSONObject generateInputSampleFromJSON(Stage stage, String minAppVersion, ServletContext context, JSONObject updatedInputSchema, InputSampleGenerationMode generationMode) throws GenerationException, JSONException {
		...
		String inputSampleStr = JsonGenerator.generation(generator, faker, prune, updatedInputSchema.toString(), stage.toString(), minAppVersion, generationMode, -1); //TODO: is it the right randomizer?
	...
	public JSONObject mergeSchema(JSONObject streamsJson, boolean validationOnly, JSONObject newSchema) throws GenerationException
	{
		try {
			...
			AddStreamsToSchema.merge(tmpSchema, streamsJson, false); //TODO: are streams required or optional?
