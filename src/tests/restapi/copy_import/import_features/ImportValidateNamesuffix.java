...
public class ImportValidateNamesuffix {
	...
	public void simulateInvalidSuffixName() throws IOException{
		...
		String response = f.importFeatureToBranch(featureToImport, featureID2, "VALIDATE", null, "-new$suffix", true, sessionToken, srcBranchID);
		Assert.assertTrue(response.contains("error"), "Feature was imported with illegal characters in name ");
	}
	...
	public void invalidSuffixName() throws IOException{
		String featureToImport = f.getFeatureFromBranch(featureID1, srcBranchID, sessionToken);
		String response = f.importFeatureToBranch(featureToImport, featureID2, "ACT", null, "-new$suffix", true, sessionToken, srcBranchID);
		Assert.assertTrue(response.contains("error"), "Feature was imported with illegal characters in name ");
	}
