...
public class CopyFeatureWithConflicts {
	...
	@Test (dependsOnMethods="addLocale", description = "Copy feature to season2 with internationalFallback - copy feature, do not copy strings")
	public void copyFeatureInternationalFallback() throws Exception{
		...
		response = f.copyFeature(featureID, rootId2, "ACT", null, "suffix2", sessionToken);
	...
	@Test (dependsOnMethods="copyFeatureInternationalFallback", description = "Copy feature to season2 with different stage - copy feature, do not copy strings")
	public void copyFeatureDifferentStage() throws Exception{
		String str = stringsApi.getString(stringID, sessionToken);
		JSONObject strJson = new JSONObject(str);
		...
		String rootId2 = f.getRootId(seasonID2, sessionToken);
		response = f.copyFeature(featureID, rootId2, "ACT", null, "suffix3", sessionToken);
		Assert.assertTrue(response.contains("newSubTreeId"), "Feature was not copied: " + response);
	...
	@Test (dependsOnMethods="copyFeatureDifferentStage", description = "Copy feature to season2 with translation - copy feature, do not copy strings")
	public void copyFeatureDifferentTranslation() throws Exception{
        //update translation
		...
		String rootId2 = f.getRootId(seasonID2, sessionToken);
		response = f.copyFeature(featureID, rootId2, "ACT", null, "suffix4", sessionToken);
		Assert.assertTrue(response.contains("newSubTreeId"), "Feature was not copied: " + response);
	...
	@Test (dependsOnMethods="copyFeatureDifferentTranslation", description = "Copy feature to season2 with different value - conflict, no string copy")
	public void copyFeatureDifferentValue() throws Exception{
		String str = stringsApi.getString(stringID, sessionToken);
		JSONObject strJson = new JSONObject(str);
		...
		String rootId2 = f.getRootId(seasonID2, sessionToken);
		response = f.copyFeature(featureID, rootId2, "VALIDATE", null, "suffix5", sessionToken);		
		response = f.copyFeature(featureID, rootId2, "ACT", null, "suffix5", sessionToken);
		Assert.assertTrue(response.contains("newSubTreeId"), "Feature was not copied: " + response);
	...
	/*
	@Test (dependsOnMethods="copyFeatureDifferentValue", description = "Copy feature to season2 with different variant - conflict, no string copy")
	public void copyFeatureDifferentVariant() throws Exception{
		//return previous value
		String str = stringsApi.getString(stringID, sessionToken);
		JSONObject strJson = new JSONObject(str);
		strJson.put("value", "Hello [[[1]]]");
		String response = stringsApi.updateString(stringID, strJson.toString(), sessionToken);
		Assert.assertFalse(response.contains("error"), "String1 was not updated in season1: " + response);
		...
		String str1 = stringsApi.getString(stringsInSeasonJson.getJSONArray("strings").getJSONObject(0).getString("uniqueId"), "BASIC", sessionToken);
		JSONObject str1Json = new JSONObject(str1);
		Assert.assertFalse(str1Json.getString("variant").equals("Hello"), "Variant for string1 in french was updated");		
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").size()==2, "String was not copied to season2");
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(1).getString("key").equals("app.hi"), "Incorrect key2 copied to season2");
	...
	*/
	private void cleanSeason2(String id) throws Exception{
		...
		for(int i=0; i< features.size(); i++){
			f.deleteFeature(features.getJSONObject(i).getString("uniqueId"), sessionToken);
		}	
