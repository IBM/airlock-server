...
public class ImportStringsDifferentStage {
	...
	public void updateTranslations() throws Exception{
        ...
        Assert.assertTrue(response.equals(""),"could not update fr translation");
        ...
        Assert.assertTrue(response.equals(""),"could not update es translation");
	...
	public void changeStages() throws Exception{
		...
		str = stringsApi.getAllStrings(seasonID2, "BASIC", sessionToken);
	...
	public void copyStringToDifferentSeason() throws Exception{
		...
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(0).getString("stage").equals("DEVELOPMENT"), "String1 stage was not updated in season2");
		...
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(1).getString("stage").equals("DEVELOPMENT"), "String2 stage was not updated in season2");
	}
	
	@Test (dependsOnMethods="copyStringToDifferentSeason", description = "Change strings stages in season2")
	public void changeStages2() throws Exception{


 		String str = stringsApi.getAllStrings(seasonID2, "BASIC", sessionToken);
		JSONObject temp = new JSONObject(str);
		JSONArray allstr = temp.getJSONArray("strings");
		for (int i=0; i<allstr.size(); i++){
			JSONObject single = allstr.getJSONObject(i);
		...
		//update ES translation in season1
	       String translationEs = FileUtils.fileToString(filePath + "strings/translationES7.txt", "UTF-8", false);
	       String response = translationsApi.updateTranslation(seasonID,"es",translationEs,sessionToken);

	}
	
	@Test (dependsOnMethods="changeStages2", description = "Copy strings from PRODUCTION to PRODUCTION")
	public void copyStringToDifferentSeason2() throws Exception{
		

		String content = stringsApi.getAllStrings(seasonID, "INCLUDE_TRANSLATIONS", sessionToken);
		Assert.assertFalse(content.contains("error"), "Strings were not copied");
	//	Assert.assertTrue(response.getJSONArray("stringsOverride").size()==2, "Strings were not recognized in conflict");
		...
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(0).getJSONObject("translations").containsKey("es"), "String1 ES translation was not found in season2");
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(0).getString("status").equals("TRANSLATION_COMPLETE"), "String1 status was not updated in season2");
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(0).getString("stage").equals("PRODUCTION"), "String1 stage was not updated in season2");
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(1).getString("key").equals("app.hi"), "Incorrect key2 copied to season2");
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(1).getJSONObject("translations").containsKey("fr"), "String2 FR translation was not found in season2");
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(1).getJSONObject("translations").containsKey("es"), "String2 ES translation was not found in season2");
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(1).getString("status").equals("TRANSLATION_COMPLETE"), "String2 status was not updated in season2");
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(1).getString("stage").equals("PRODUCTION"), "String2 stage was not updated in season2");
	}
