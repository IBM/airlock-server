...
public class CopyStringsIOSToAndroidLocales {
	...
	public void addComponents() throws Exception{
		 ...
		 seasonID2 = s.addSeason(productID, "{\"minVersion\": \"5.0\"}", sessionToken);
	      ...
	      //add iOS specific locales to season1       
		String response = translationsApi.addSupportedLocales(seasonID,"he",sessionToken);
	    Assert.assertTrue(response.equals(""),"could not add he");
		response = translationsApi.addSupportedLocales(seasonID,"zh_Hans",sessionToken);
	    Assert.assertTrue(response.equals(""),"could not add zh_Hans");
		response = translationsApi.addSupportedLocales(seasonID,"zh_Hant",sessionToken);
	    Assert.assertTrue(response.equals(""),"could not add zh_Hant");

		 
		//add Android specific locales to season2
		response = translationsApi.addSupportedLocales(seasonID2,"iw",sessionToken);
        Assert.assertTrue(response.equals(""),"could not add iw");
		response = translationsApi.addSupportedLocales(seasonID2,"zh_CN",sessionToken);
        Assert.assertTrue(response.equals(""),"could not add zh_CN");
		response = translationsApi.addSupportedLocales(seasonID2,"zh_TW",sessionToken);
	...
	public void addTranslations() throws Exception{
       ...
       String  response = translationsApi.markForTranslation(seasonID,new String[]{stringID},sessionToken);
        ...
        response = translationsApi.updateTranslation(seasonID,"he",translationsJson.getJSONObject("iw").toString(),sessionToken);
        Assert.assertTrue(response.equals(""),"could not add translation for he");
        response = translationsApi.updateTranslation(seasonID,"zh_Hans",translationsJson.getJSONObject("zh_CN").toString(),sessionToken);
        Assert.assertTrue(response.equals(""),"could not add translation for zh_Hans");
        response = translationsApi.updateTranslation(seasonID,"zh_Hant",translationsJson.getJSONObject("zh_TW").toString(),sessionToken);
        Assert.assertTrue(response.equals(""),"could not add translation for zh_Hant");
	...
	public void copyStringToDifferentSeason() throws Exception{
		...
		String response = stringsApi.copyStrings(new String[]{stringID}, seasonID2, "ACT", false, sessionToken);
		...
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(0).getJSONObject("translations").containsKey("iw"), "String1 IW translation was not found in season2");
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(0).getJSONObject("translations").containsKey("zh_CN"), "String1 zh_CN translation was not found in season2");
		Assert.assertTrue(stringsInSeasonJson.getJSONArray("strings").getJSONObject(0).getJSONObject("translations").containsKey("zh_TW"), "String1 zh_TW translation was not found in season2");
