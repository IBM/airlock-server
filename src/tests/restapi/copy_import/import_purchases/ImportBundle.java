...
public class ImportBundle {
	 ...
	 * import bundle to s1
	 * create season s2 and try to import the bundle to it
	 * create new product and s3 and try to import the bundle to it 	
	 */
	...
	@Test (dependsOnMethods="createComponemts", description="Import bundle within the same season under root")
	public void importBundleSameSeason() throws IOException, JSONException{
		...
		String bundleToImport = purchasesApi.getPurchaseItemFromBranch(bundleID, srcBranchID, sessionToken);
		
		String response = f.importFeatureToBranch(bundleToImport, rootId, "ACT", null, suffix, true, sessionToken, destBranchID);
		Assert.assertTrue(response.contains("newSubTreeId"), "Bundle was not imported: " + response);
		
		JSONArray entitlementsArr = purchasesApi.getPurchasesBySeasonFromBranch(seasonID1, destBranchID, sessionToken);
		Assert.assertTrue(entitlementsArr.size() == 4, "wrong entitlements number after import");
	...
	@Test (dependsOnMethods="importBundleSameSeason", description="import bundle to another season under root")
	public void importFeatureOtherSeason() throws Exception{
		...
		String importedBundleIDInNewSeason = entitlementsArr.getJSONObject(3).getString("uniqueId");
		Assert.assertTrue(entitlementsArr.getJSONObject(3).getString("name").equals("bundle"+suffix), "wrong newentitlement name");
		Assert.assertFalse(importedBundleIDInNewSeason.equals(bundleIDInNewSeason), "wrong new entitlement id");
		Assert.assertTrue(entitlementsArr.getJSONObject(3).getJSONArray("includedEntitlements").size() == 2, "wrong includedEntitlements size");
	...
	@Test (dependsOnMethods="importFeatureOtherSeason", description="import bundle to another product")
	public void importBundleDifferentProduct() throws IOException, JSONException{
		...
		String response = f.importFeatureToBranch(bundleToImport, rootId, "ACT", null, suffix, true, sessionToken, destBranchID);
		Assert.assertTrue(response.contains("error") && response.contains("entitlement") , "bundle was imported even when its included entitlement is missing");
		
		//add entitlements to season3
		String e1ToImport = purchasesApi.getPurchaseItemFromBranch(entitlementID1, srcBranchID, sessionToken);
		response = f.importFeatureToBranch(e1ToImport, rootId, "ACT", null, "", true, sessionToken, destBranchID);
		Assert.assertTrue(response.contains("newSubTreeId") && !response.contains("error") , "entitlement1 was not imported : "+ response);
		
		String e2ToImport = purchasesApi.getPurchaseItemFromBranch(entitlementID2, srcBranchID, sessionToken);
		response = f.importFeatureToBranch(e2ToImport, rootId, "ACT", null, "", true, sessionToken, destBranchID);
		...
		//import bundle again
		response = f.importFeatureToBranch(bundleToImport, rootId, "ACT", null, suffix, true, sessionToken, destBranchID);
		Assert.assertFalse(response.contains("error"), "bundle was not imported even when its included entitlement is exist:" +response);
		
		entitlementsArr = purchasesApi.getPurchasesBySeasonFromBranch(seasonID3, destBranchID, sessionToken);
		Assert.assertTrue(entitlementsArr.size() == 3, "wrong entitlements number after import");
