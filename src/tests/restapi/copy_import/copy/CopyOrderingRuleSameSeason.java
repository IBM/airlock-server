...
public class CopyOrderingRuleSameSeason {
	...
	@Test (description = "F3, F1 -> OR, F2, copy OR")
	public void noCopy() throws JSONException, IOException{
		...
		String featureID3 = f.addFeatureToBranch(seasonID, srcBranchID, json.toString(), "ROOT", sessionToken);
		...
		//copy OR to F3
		String response = f.copyItemBetweenBranches(orderingRuleID, featureID3, "VALIDATE", null, "suffix1", sessionToken, srcBranchID, srcBranchID);
		Assert.assertTrue(response.contains("error"), "Stand alone Ordering rule was copied: " + response);
	...
	public void copy1() throws JSONException, IOException{
		...
		JSONObject responseJson = new JSONObject (f.copyItemBetweenBranches(parentID, rootId, "VALIDATE", null, null, sessionToken, srcBranchID, srcBranchID));
		Assert.assertTrue(responseJson.getJSONArray("illegalName").size() > 0, "Ordering rules validation incorrect: " + responseJson.toString());
		
		String response = f.copyItemBetweenBranches(parentID, rootId, "ACT", null, "suffix1", sessionToken, srcBranchID, srcBranchID);
	...
	public void copy3() throws JSONException, IOException{
		...
		String rootId = f.getBranchRootId(seasonID, srcBranchID, sessionToken);
		String response = f.copyItemBetweenBranches(mtxID, rootId, "ACT", null, "suffix1", sessionToken, srcBranchID, srcBranchID);
		Assert.assertFalse(response.contains("error"), "Can't copy feature: " + response);
	...
	public void copy4() throws JSONException, IOException{
		...
		Assert.assertFalse(ORMTXID.contains("error"), "Can't add orderingRule  " + ORMTXID);
