...
public class CopyFeatureInProductionStageSameSeason {
	...
	public void copyFeature() throws IOException{
		
		String response = f.copyItemBetweenBranches(mixIdToCopy, featureID2, "ACT", null, "suffix1", sessionToken, srcBranchID, srcBranchID);
		Assert.assertTrue(response.contains("newSubTreeId"), "Feature was not copied: " + response);
	}
	
	
	@Test (dependsOnMethods="copyFeature", description="Validate feature tree that should be converted to development")
