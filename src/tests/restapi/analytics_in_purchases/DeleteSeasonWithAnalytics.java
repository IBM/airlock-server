...
public class DeleteSeasonWithAnalytics {
	...
	*/
	...
	public void addExperiment () throws Exception {
		...
		variantID2 = addVariant("variant2", "branch1");
	...
	@Test (dependsOnMethods="addExperiment", description ="Get experiment quota") 
	public void getQuota () throws IOException, JSONException {
	...
	@Test (dependsOnMethods="addExperiment", description ="Add items to analytics in both seasons") 
	public void reportToAnalytics () throws IOException, JSONException, InterruptedException {
		...
		jsonE.put("name", "EntitlementInSeason1");
		String featureID1 = purchasesApi.addPurchaseItem(seasonID1, jsonE.toString(), "ROOT", sessionToken);
		an.addFeatureToAnalytics(featureID1, BranchesRestApi.MASTER, sessionToken);
		jsonE.put("name", "EntitlementInBranch1");
		String featureID2 = purchasesApi.addPurchaseItemToBranch(seasonID1, branchID1, jsonE.toString(), "ROOT", sessionToken);
		an.addFeatureToAnalytics(featureID2, branchID1, sessionToken);
		
		//add entitlements to analytics in season2
		jsonE.put("name", "EntitlementInSeason2");
		String featureID3 = purchasesApi.addPurchaseItem(seasonID2, jsonE.toString(), "ROOT", sessionToken);
		an.addFeatureToAnalytics(featureID3, BranchesRestApi.MASTER, sessionToken);
		jsonE.put("name", "EntitlementInBranch2");
		String featureID4 = purchasesApi.addPurchaseItemToBranch(seasonID2, branchID2, jsonE.toString(), "ROOT", sessionToken);
		an.addFeatureToAnalytics(featureID4, branchID2, sessionToken);
		
		String analyticsReponse = an.getExperimentGlobalDataCollection(experimentID, sessionToken);
		Assert.assertTrue(analyticsFeaturesInExperiment(analyticsReponse)==4, "Incorrect number of entitlements in experiment analytics");
		...
		String err = validateInputFieldsList(experimentAnalytics, new String[]{"context.device.localeCountryCode", "context.device.locale", "context.device.localeLanguage", "context.device.datetime"});
	...
	@Test (dependsOnMethods="reportToAnalytics", description ="Delete first season") 
	public void deleteSeason () throws IOException, JSONException, InterruptedException {
		String dateFormat = an.setDateFormat();
		
		int resp = s.deleteSeason(seasonID1, sessionToken);
		Assert.assertTrue(resp == 200, "Season1 was not deleted");

		//get quota, it should be set to season2
		JSONObject jsonResp = new JSONObject(an.getExperimentQuota(experimentID, sessionToken));
		Assert.assertTrue(jsonResp.getInt("analyticsQuota")==3, "Incorrect quota in experiment after delete season");

		String analyticsReponse = an.getExperimentGlobalDataCollection(experimentID, sessionToken);
		Assert.assertTrue(analyticsFeaturesInExperiment(analyticsReponse)==2, "Incorrect number of features in experiment analytics");
		...
		Assert.assertTrue(responseDev.code ==200, "Runtime feature file was not updated");
		JSONObject experimentAnalytics = getExperimentAnalytics(responseDev.message);
		...
		err = validateInputFieldsList(experimentAnalytics, new String[]{"context.device.localeCountryCode", "context.device.datetime"});
		Assert.assertTrue(err != null, "Deleted input field found in experiment analytics");		
		err = validateEntitlementsToAnalytics(experimentAnalytics, new String[]{"ns1.EntitlementInSeason1", "ns1.EntitlementInBranch1"});
		Assert.assertTrue(err != null, "Deleted entitlements found in experiment analytics");
	}
	...
	private String validateEntitlementsToAnalytics(JSONObject experimentAnalytics, String[] expectedFeatures) {
