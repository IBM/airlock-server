...
public class ExceedQuotaDuringExperimentVariantCreate {
	...
	public void addToAnalytics() throws Exception{
		...
		//add entitlement in development to master and report it in branch
		...
		entitlementID1 = purchasesApi.addPurchaseItem(seasonID, jsonF.toString(), "ROOT", sessionToken);
		...
		Assert.assertTrue(getDevelopmentItemsReportedToAnalytics(respWithQuota)==5, "Incorrect number of development items"); //entitlement1+attribute+inputfield
		Assert.assertTrue(getProductionItemsReportedToAnalytics(respWithQuota)==2, "Incorrect number of production items"); //entitlement1+attribute+inputfield
		
		//move entitlement to prod in master
		jsonF = new JSONObject(purchasesApi.getPurchaseItem(entitlementID1, sessionToken));
		jsonF.put("stage", "PRODUCTION");
		response = purchasesApi.updatePurchaseItem(seasonID, entitlementID1, jsonF.toString(), sessionToken);
