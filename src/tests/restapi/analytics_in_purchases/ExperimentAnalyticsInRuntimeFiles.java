...
public class ExperimentAnalyticsInRuntimeFiles {
	...
	protected String entitlementIDProd;
	protected String entitlementIDDev;
	...
	public void addToAnalytics() throws Exception{
		...
		jsonE.put("name", "Eprod");
		entitlementIDProd = purchasesApi.addPurchaseItemToBranch(seasonID1, BranchesRestApi.MASTER, jsonE.toString(), "ROOT", sessionToken);
		Assert.assertFalse(entitlementIDProd.contains("error"), "Entitlement was not added to the season" + entitlementIDProd);
		
		response = an.addFeatureToAnalytics(entitlementIDProd, BranchesRestApi.MASTER, sessionToken);
		...
		configIDProd = purchasesApi.addPurchaseItemToBranch(seasonID1, BranchesRestApi.MASTER, jsonConfig.toString(), entitlementIDProd, sessionToken);
		...
		response = an.addAttributesToAnalytics(entitlementIDProd, BranchesRestApi.MASTER, attributes.toString(), sessionToken);
		Assert.assertFalse(response.contains("error"), "Entitlement was not added to analytics" + response);

		//add entitlement in development  in master
		jsonE = new JSONObject(entitlement1);
		jsonE.put("stage", "DEVELOPMENT");
		jsonE.put("name", "Edev");
		entitlementIDDev = purchasesApi.addPurchaseItemToBranch(seasonID1, BranchesRestApi.MASTER, jsonE.toString(), "ROOT", sessionToken);
		Assert.assertFalse(entitlementIDDev.contains("error"), "Entitlement was not added to the season" + entitlementIDDev);
		
		response = an.addFeatureToAnalytics(entitlementIDDev, BranchesRestApi.MASTER, sessionToken);
		Assert.assertFalse(response.contains("error"), "Incorrect globalDataCollection response");	
		...
		configIDDev = purchasesApi.addPurchaseItemToBranch(seasonID1, BranchesRestApi.MASTER, jsonConfigDev.toString(), entitlementIDDev, sessionToken);
		...
		response = an.addAttributesToAnalytics(entitlementIDDev, BranchesRestApi.MASTER, attributes.toString(), sessionToken);
		Assert.assertFalse(response.contains("error"), "Entitlement was not added to analytics" + response);
	}
	...
	public void addExperiment() throws Exception{
		...
		err = validateFeaturesToAnalytics(experimentAnalytics, new String[]{"ns1.Edev", "ns1.Eprod"});
		Assert.assertTrue(err == null, err);
		
		JSONArray entitlementAttributes = new JSONArray();
		JSONObject entitlementAtt1 = new JSONObject();
		entitlementAtt1.put("name", "ns1.Edev");
		entitlementAtt1.put("attributes", new String[]{"colorDev"});
		entitlementAttributes.add(entitlementAtt1);
		JSONObject entitlementAtt2 = new JSONObject();
		entitlementAtt2.put("name", "ns1.Eprod");
		entitlementAtt2.put("attributes", new String[]{"colorProd"});
		entitlementAttributes.add(entitlementAtt2);
				
		err = validateFeatureAttributesForAnalytics(experimentAnalytics, entitlementAttributes);
		Assert.assertTrue(err == null, err);
		...
		err = validateFeaturesToAnalytics(experimentAnalytics, new String[]{"ns1.Eprod"});
		Assert.assertTrue(err == null, err);
		
		entitlementAttributes = new JSONArray();
		entitlementAtt2 = new JSONObject();
		entitlementAtt2.put("name", "ns1.Eprod");
		entitlementAtt2.put("attributes", new String[]{"colorProd"});
		entitlementAttributes.add(entitlementAtt2);
	...
	public void addBranchesAndVariantsInProduction() throws Exception{
		...
		//validate prod experiment analytics
		RuntimeRestApi.DateModificationResults responseProd = RuntimeDateUtilities.getProductionBranchFileDateModification(m_url, productID, seasonID1, BranchesRestApi.MASTER, dateFormat, sessionToken);
		Assert.assertTrue(responseProd.code ==200, "Runtime development file was not updated");
		experimentAnalytics = getExperimentAnalytics(responseProd.message);
	...
	public void addAnalyticsToBranchInDevelopment() throws Exception{
		...
		String entitlement1 = FileUtils.fileToString(filePath + "purchases/inAppPurchase2.txt", "UTF-8", false);
		JSONObject jsonE = new JSONObject(entitlement1);
		jsonE.put("stage", "PRODUCTION");
		jsonE.put("name", "EprodInDevBranch");
		String entitlementID = purchasesApi.addPurchaseItemToBranch(seasonID1, branchID3Dev, jsonE.toString(), "ROOT", sessionToken);
		Assert.assertFalse(entitlementID.contains("error"), "Entitlement was not added to the season" + entitlementID);
		
		response = an.addFeatureToAnalytics(entitlementID, branchID3Dev, sessionToken);
		Assert.assertFalse(response.contains("error"), "Incorrect globalDataCollection response");	
		...
		String configID = purchasesApi.addPurchaseItemToBranch(seasonID1, branchID3Dev, jsonConfig.toString(), entitlementID, sessionToken);
		...
		response = an.addAttributesToAnalytics(entitlementID, branchID3Dev, attributes.toString(), sessionToken);
		Assert.assertFalse(response.contains("error"), "Entitlement was not added to analytics" + response);
		...
		err = validateFeaturesToAnalytics(experimentAnalytics, new String[]{"ns1.Edev", "ns1.Eprod", "ns1.EprodInDevBranch"});
		Assert.assertTrue(err == null, err);
		...
		featureAtt3.put("name", "ns1.EprodInDevBranch");
		...
		RuntimeRestApi.DateModificationResults responseProd = RuntimeDateUtilities.getProductionBranchFileDateModification(m_url, productID, seasonID1, BranchesRestApi.MASTER, dateFormat, sessionToken);
		Assert.assertTrue(responseProd.code ==304, "Runtime development file was not updated");
	...
	public void addAnalyticsToSecondMaster() throws Exception{
		...
		jsonF.put("name", "EprodInSecMaster");
		String EntitlementID = purchasesApi.addPurchaseItemToBranch(seasonID2, BranchesRestApi.MASTER, jsonF.toString(), "ROOT", sessionToken);
		Assert.assertFalse(EntitlementID.contains("error"), "Entitlement was not added to the season" + EntitlementID);
		
		response = an.addFeatureToAnalytics(EntitlementID, BranchesRestApi.MASTER, sessionToken);
		Assert.assertFalse(response.contains("error"), "Incorrect globalDataCollection response");	
		...
		String configID = purchasesApi.addPurchaseItemToBranch(seasonID2, BranchesRestApi.MASTER, jsonConfig.toString(), EntitlementID, sessionToken);
		...
		response = an.addAttributesToAnalytics(EntitlementID, BranchesRestApi.MASTER, attributes.toString(), sessionToken);
		...
		err = validateFeaturesToAnalytics(experimentAnalytics, new String[]{"ns1.Edev", "ns1.Eprod", "ns1.EprodInDevBranch", "ns1.EprodInSecMaster"});
		Assert.assertTrue(err == null, err);
		...
		featureAtt4.put("name", "ns1.EprodInSecMaster");
		...
		err = validateFeaturesToAnalytics(experimentAnalytics, new String[]{"ns1.Eprod", "ns1.EprodInSecMaster"});
		Assert.assertTrue(err == null, err);
	...
	*/
		...
		//move Eprod to development
		JSONArray featuresInSeason = purchasesApi.getPurchasesBySeason(seasonID2, sessionToken);
		String err = updateEntitlementStage(featuresInSeason, "Eprod", "DEVELOPMENT");
		Assert.assertTrue(err == null, err);
		err = updateEntitlementStage(featuresInSeason, "Edev", "PRODUCTION");
		Assert.assertTrue(err == null, err);
		...
		err = validateFeaturesToAnalytics(experimentAnalytics, new String[]{"ns1.Edev", "ns1.EprodInSecMaster", "ns1.Eprod"});
		Assert.assertTrue(err == null, err);
	...
	private String validateFeatureAttributesForAnalytics(JSONObject experimentAnalytics, JSONArray expectedFeaturesAttributes) {
		...
		try {
			...
			if (featuresAttributesForAnalytics.size() != expectedFeaturesAttributes.size())
				return "wrong number of entitlements that report attribute to analytics.";
	...
	private String updateEntitlementStage(JSONArray features, String eName, String eStage) throws JSONException, IOException{
		for (int i=0; i<features.size(); i++){
			JSONObject feature = features.getJSONObject(i);
			if (feature.getString("name").equals(eName)){
				String featureId = feature.getString("uniqueId");
				JSONObject json = new JSONObject(purchasesApi.getPurchaseItem(featureId, sessionToken));
				json.put("stage", eStage);
				String response = purchasesApi.updatePurchaseItem(seasonID2, featureId, json.toString(), sessionToken);
