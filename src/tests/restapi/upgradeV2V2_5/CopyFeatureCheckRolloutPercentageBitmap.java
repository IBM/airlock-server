...
public class CopyFeatureCheckRolloutPercentageBitmap {
	...
	@Test (description="Copy a feature inside the same old season")
	public void copyFeatureInsideOldSeason() throws IOException, JSONException{
		...
		String response = f.copyFeature(featureID1, rootId, "ACT", null, "suffix1", sessionToken);
	...
	@Test (dependsOnMethods ="copyFeatureInsideOldSeason",  description="Copy a feature from old season to new season")
	public void copyFeatureFromOldToNewSeason() throws IOException, JSONException{
		...
		String response = f.copyFeature(featureID1, rootId, "ACT", null, "suffix2", sessionToken);
		Assert.assertTrue(response.contains("newSubTreeId"), "Feature was not copied: " + response);
	...
	@Test (dependsOnMethods ="copyFeatureFromOldToNewSeason",  description="Copy a feature from new season to old season with incorrect percentage")
	public void copyFeatureFromNewToOldSeasonInvalidPercentage() throws IOException, JSONException{
		...
		String rootId = f.getRootId(seasonID, sessionToken);
		String response = f.copyFeature(featureToCopyID, rootId, "ACT", null, "suffix3", sessionToken);
		Assert.assertTrue(response.contains("error"), "Feature was copied with incorrect rolloutPercentage: " + response);
		
	}
	
	@Test (dependsOnMethods ="copyFeatureFromNewToOldSeasonInvalidPercentage",  description="Copy a feature from new season to old season")
	public void copyFeatureFromNewToOldSeason() throws IOException, JSONException{
		JSONArray features = f.getFeaturesBySeason(seasonID2, sessionToken);
		String featureToCopyID = features.getJSONObject(0).getString("uniqueId");
		String featureToCopy = f.getFeature(featureToCopyID, sessionToken);
		JSONObject jsonF = new JSONObject(featureToCopy);
