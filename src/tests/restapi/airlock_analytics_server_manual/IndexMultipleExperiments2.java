...
public class IndexMultipleExperiments2 {
	...
	@Test (dependsOnMethods="createExperiment1", description ="Add disabled production experiment") 
	public void createExperiment2 () throws Exception {
			...
			json.put("enabled", true);
			json.put("indexExperiment", JSONObject.NULL);
		   ...
		   //range contains 10 buckets
			...
			String response = expApi.updateExperiment(experimentID2, expJson1.toString(), sessionToken);
			Assert.assertFalse(response.contains("error"), "Experiment2 was not updated: " + response);

			String aasResponse = expAnalyticsApi.getExperiment(experimentID2, sessionToken);
			Assert.assertFalse(aasResponse.contains("not found"), "Experiment was not published to aas server");
	...
	@Test (dependsOnMethods="createExperiment2", description ="Add disabled production experiment") 
