...
public class CountersChangeStageInBranch {
	...
	@Test (dependsOnMethods="addBranch", description="add feature in development in master to analytics, checkout to branch and move to production")
	public void addFeatureToAnalytics() throws Exception{
		...
		//move feature to production in branch
		...
		Assert.assertFalse(featureID1.contains("error"), "Feature was not updated in branch " + featureID1);
		...
		//move feature to development in branch
		feature1 = f.getFeatureFromBranch(featureID1, branchID, sessionToken);
	...
	@Test (dependsOnMethods="addFeatureToAnalytics", description="add feature attributes in development in master to analytics, checkout to branch and move to production")
	public void addAttributesToAnalytics() throws Exception{
		...
		String response = an.getGlobalDataCollection(seasonID, "MASTER", "BASIC", sessionToken);
	    ...
	    configID1 = f.updateFeatureInBranch(seasonID, branchID, configID1, jsonCR1.toString(), sessionToken);
	...
	public void deleteConfigurationInMaster() throws Exception{
		...
		int response = f.deleteFeature(configID1,  sessionToken);
	...
	public void deleteFeatureFromMaster() throws Exception{
		...
		Assert.assertTrue(response == 200, "Feature was not deleted from master " + response);
