...
public class ExceedQuotaAddToGlobalDataCollection {
	...
	@Test (dependsOnMethods="addToAnalytics", description="Update to production 1 input field, 1 feature, 1 attribute")
	public void updateFieldsToProduction() throws Exception{
        ...
        response = f.updateFeatureInBranch(seasonID, branchID, featureID1, jsonF.toString(), sessionToken);
        ...
        response = f.updateFeatureInBranch(seasonID, branchID, configID1, jsonCR.toString(), sessionToken);
        Assert.assertFalse(response.contains("error"), "Configuration was not updated to production" + response);
	...
	@Test (dependsOnMethods="updateFieldsToProduction", description="Add Production input field - exceeds the quota")
	public void addInputFieldOverQuota() throws JSONException{		
		//add production input field
		String response = an.getGlobalDataCollection(seasonID, branchID, "BASIC", sessionToken);
		String input = an.addInputFieldsToAnalytics(response, "context.device.connectionType");
		response = an.updateGlobalDataCollection(seasonID, branchID, input, sessionToken);
	...
	public void addFeatureToBranchOverQuota() throws JSONException{
		//add new feature in production
		String response = an.getGlobalDataCollection(seasonID, branchID, "BASIC", sessionToken);	
		String input = an.addFeatureOnOff(response, configID1);
		response = an.updateGlobalDataCollection(seasonID, branchID, input, sessionToken);
	...
	@Test (dependsOnMethods="addFeatureToBranchOverQuota", description="Add attribute - exceeds the quota")
