...
public class ExceedQuotaAddSimultaneouslyToGlobalDataCollection {
	...
	/*
	 *  add all items for analytics at once over quota
	 */
	...
	public void addToAnalytics() throws Exception{
		...
		input = an.addFeaturesAttributesToAnalytics(input, featureID1, attributes);
		
		
		//add all items to analytics at once
		response = an.updateGlobalDataCollection(seasonID, branchID, input, sessionToken);
		Assert.assertTrue(response.contains("The maximum number"), "Feature was not added to analytics" + response);
	}
	


	@Test (dependsOnMethods="addToAnalytics", description="Set quota to 5")
	...
	@Test (dependsOnMethods="updateQuota2", description="Add items to analytics below quota")
	public void addToAnalytics2() throws Exception{
		//add input field
		String response = an.getGlobalDataCollection(seasonID, branchID, "VERBOSE", sessionToken);
		String input = an.addInputFieldsToAnalytics(response, "context.device.locale");
		...
		//add all items to analytics at once
		response = an.updateGlobalDataCollection(seasonID, branchID, input, sessionToken);
		Assert.assertFalse(response.contains("The maximum number"), "Feature was not added to analytics" + response);
