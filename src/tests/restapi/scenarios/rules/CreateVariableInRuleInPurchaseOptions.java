...
public class CreateVariableInRuleInPurchaseOptions {
	...
	public void addGlobalVar() throws JSONException, IOException{
		...
		Assert.assertFalse(entitlementID.contains("error"), "Can't create entitlement");
		
		JSONObject poJson = new JSONObject(purchaseOptions);
		poJson.put("name", "PO1");
		String purchaseOptionsID = purchasesApi.addPurchaseItem(seasonID, poJson.toString(), entitlementID, sessionToken);
		Assert.assertFalse(purchaseOptionsID.contains("error"), "Can't create purchaseOptions");
		...
		String configID = purchasesApi.addPurchaseItem(seasonID, jsonCR.toString(), purchaseOptionsID, sessionToken);
	...
	public void addVarInUtility() throws IOException, JSONException{
		...
		JSONObject poJson = new JSONObject(purchaseOptions);
		poJson.put("name", "PO55");
		String purchaseOptionsID = purchasesApi.addPurchaseItem(seasonID, poJson.toString(), entitlementID, sessionToken);
		Assert.assertFalse(purchaseOptionsID.contains("error"), "Can't create purchaseOptions");
		
		JSONObject jsonCR = new JSONObject(configuration);
		jsonCR.put("name", "CR55");	
		jsonCR.put("configuration", confString);
		String configID = purchasesApi.addPurchaseItem(seasonID, jsonCR.toString(), purchaseOptionsID, sessionToken);
		Assert.assertFalse(configID.contains("error"), "Configuraton was not created: " + configID);
	}
