...
public class DeleteConfigurationRule {
	...
	@Test (dependsOnMethods="createConfigurationRule", description = "Delete configuration rule")
	public void deleteConfigurationRule() throws IOException, JSONException{
		int responseCode = f.deleteFeature(configRuleID, sessionToken);
		Assert.assertEquals(responseCode, 200, "Feature deleted code 200 was expected, but " + responseCode + " was received");
	}
	
	@Test (dependsOnMethods="deleteConfigurationRule", description = "Check that configuration rule was deleted from parent feature")
	public void validateParentFeature() throws IOException, JSONException{
		String feature = f.getFeature(featureID, sessionToken);
		JSONObject json = new JSONObject(feature);
		JSONArray children = json.getJSONArray("features");
		Assert.assertTrue(children.size()==0, "Configuration rule was not deleted from feature");
