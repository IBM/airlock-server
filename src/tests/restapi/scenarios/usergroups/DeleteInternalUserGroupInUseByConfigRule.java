...
public class DeleteInternalUserGroupInUseByConfigRule {
	...
	public void addConfigurationRule() throws Exception{
		...
		String configRuleID = f.addFeature(seasonID, json.toString(), featureId, sessionToken );
		Assert.assertFalse(configRuleID.contains("error"), "could  not add config rule with new user group: " + configRuleID );
	}
	
	@Test (dependsOnMethods = "addConfigurationRule", description = "Delete usergroup in use by configuration rule")
	public void deleteUserGroups() throws Exception{
		...
		String res = ug.setUserGroups(productID, json.toString(), sessionToken);
		Assert.assertTrue(res.contains("cannot be deleted") && res.contains("ns1.CR1"), "Removed usergroup in use");
	}
