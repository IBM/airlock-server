...
public class WebhookSimpleScenarios {
	...
	@Test (description="create simple that shouldnt accept admin change", enabled=true)
	...
	@Test (description="create simple webhook that should accept runtime change", enabled=true)
	public void sendRuntimeTrue() throws JSONException, InterruptedException, IOException {
		...
		f.addFeature(seasonID, fJson.toString(), "ROOT", sessionToken);
		thread3.join(4000);
	...
	@Test (description="create simple webhook that should not accept runtime change", enabled=true)
