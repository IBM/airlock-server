...
public class UpdateFeatureAndCompareParameters {
	...
	public void init(String url, String analyticsUrl, String translationsUrl, String configPath, String sToken, String userName, String userPassword, String appName, String productsToDeleteFile, String notify) throws Exception{
		...
		notification.followFeature(featureID);
	...
	public void testUpdateNamespace() throws JSONException, IOException{
			String field = updateStringField("stage", "DEVELOPMENT");
			field = updateStringField("namespace", "NewNamespace");
	...
	@Test 
	public void testStage() throws JSONException, IOException{
			String field = updateStringField("stage", "PRODUCTION");
			Assert.assertTrue(field.equals("PRODUCTION"), "Failed to updated parameter \"stage\".");
	}
	...
	@Test 
	public void testDefaultIfAirlockSystemIsDown() throws JSONException, IOException{
			boolean field = updateBooleanField("defaultIfAirlockSystemIsDown", false);
			Assert.assertTrue(!field, "Failed to updated parameter \"defaultIfAirlockSystemIsDown\".");
	}
	...
	public void testLastModified() throws JSONException, IOException{
			...
			//lastModified is only sent to make sure that the update is the latest.
			//Its passed value is ignored and a new value is generated
	}
	...
	public void testCreationDate() throws JSONException, IOException{
		String feature = f.getFeature(featureID, sessionToken);
		JSONObject json = new JSONObject(feature);
		long currentDate = json.getLong("creationDate")-10;
		Assert.assertNotNull(currentDate, "The field creationDate was not initialized.");
		json.put("creationDate", currentDate);
		String response = f.updateFeature(seasonID, featureID, json.toString(), sessionToken);
		Assert.assertTrue(response.contains("error"), "Test should fail, but instead passed: " + response );
	}
	...
	public void testRolloutPercentage() throws JSONException, IOException{
			String feature = f.getFeature(featureID, sessionToken);
			JSONObject json = new JSONObject(feature);
			json.put("rolloutPercentage", 30.0);
			...
			Assert.assertTrue(json.getDouble("rolloutPercentage")==30.0, "Failed to updated parameter \"rolloutPercentage\".");
	}
	...
	public void testType() throws JSONException, IOException{
			String feature = f.getFeature(featureID, sessionToken);
			JSONObject json = new JSONObject(feature);
			json.put("type", "MUTUAL_EXCLUSION_GROUP");
			String response = f.updateFeature(seasonID, featureID, json.toString(), sessionToken);
			Assert.assertTrue(response.contains("error"), "Test should fail, but instead passed: " + response );
	...
	public void testInternalUserGroups() throws Exception{
		...
		f.updateFeature(seasonID, featureID, obj.toString(), sessionToken);
		feature = f.getFeature(featureID, sessionToken);
	...
	@Test 
	public void testAdditionalInfo() throws JSONException{
		String feature = f.getFeature(featureID, sessionToken);
		JSONObject obj = new JSONObject(feature);
		JSONObject  additionalInfo = (JSONObject)obj.get("additionalInfo");
	...
	public void updateRule() throws JSONException, IOException{
		...
		feature = f.getFeature(featureID, sessionToken);
		obj = new JSONObject(feature);
		Assert.assertTrue(obj.getJSONObject("rule").getString("ruleString").equals("true"));
		Assert.assertFalse(obj.getJSONObject("rule").has("force"), "The server returned rule parameter \"force\".");
	...
	private long updateDateField(String name, long value) throws JSONException, IOException{
		String feature = f.getFeature(featureID, sessionToken);
		JSONObject json = new JSONObject(feature);
		long currentDate = json.getLong(name)-10;
...
*/
	@AfterTest
	private void reset(){
		f.unfollowFeature(featureID, sessionToken);
