...
public class ConfigurationRuleStagesMTX {
...
/*
F -> CRMTX->(CR1 + CR2)
	Create dev->prod+dev, update to  prod->prod+dev
	Create dev->dev+dev, update to dev->prod+dev
	Create prod->prod+prod, update to dev->prod+prod
	Create prod->prod+prod, update to dev->dev+prod as tree

 */

	@Test (description = "Create dev->prod+dev, update to  prod->prod+dev")
	...
	@Test (description = "Create prod->prod+prod, update to dev->dev+prod as tree")
	public void test4() throws JSONException, IOException, InterruptedException{
			...
			JSONObject jsonParent = new JSONObject(parentFeature);
			jsonParent.put("stage", "DEVELOPMENT");
			jsonParent.getJSONArray("configurationRules").getJSONObject(0).getJSONArray("configurationRules").getJSONObject(0).put("stage", "DEVELOPMENT");
			String response = f.updateFeature(seasonID, parentID, jsonParent.toString(), sessionToken);
			Assert.assertFalse(response.contains("error"), "feature was not updated: " + response);
