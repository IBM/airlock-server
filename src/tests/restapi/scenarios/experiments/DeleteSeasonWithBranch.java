...
public class DeleteSeasonWithBranch {
	...
	* CAN delete a branch when features in production if it is not used by variant
	*/
	...
	@Test (dependsOnMethods="addSeason", description ="delete a branch with features in production if it is not used by variant") 
	public void deleteBranchWithProductionFeatures () throws Exception {
		...
		String response = f.addFeatureToBranch(seasonID2, branchID, json.toString(), "ROOT", sessionToken);
		Assert.assertFalse(response.contains("error"), "Can't add feature in production " + response);
