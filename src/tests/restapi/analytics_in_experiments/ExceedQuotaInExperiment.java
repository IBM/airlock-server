...
public class ExceedQuotaInExperiment {
	...
	public void addToAnalytics() throws Exception{
		...
		featureID1 = f.addFeatureToBranch(seasonID, "MASTER", jsonF.toString(), "ROOT", sessionToken);
		Assert.assertFalse(featureID1.contains("error"), "Feature was not added to the season" + featureID1);
		
		response = an.addFeatureToAnalytics(featureID1, "MASTER", sessionToken);
		...
		configID1 = f.addFeatureToBranch(seasonID, "MASTER", jsonConfig.toString(), featureID1, sessionToken);
		...
		response = an.addAttributesToAnalytics(featureID1, "MASTER", attributes.toString(), sessionToken);
	...
	@Test (dependsOnMethods="addInputFieldOverQuotaFromBranch", description="Add Production feature - exceeds the quota")
	public void addFeatureOverQuotaFromBranch() throws JSONException, IOException{
		//add new feature in production to featureID1
	...
	@Test (dependsOnMethods="addFeatureOverQuotaFromBranch", description="Add attribute - exceeds the quota")
	public void addAttributeOverQuotaFromBranch() throws JSONException{	
		//String resp = an.getGlobalDataCollection(seasonID, branchID, "BASIC", sessionToken);
	...
	@Test (dependsOnMethods="addAttributeOverQuotaFromBranch", description="Add checked out feature over quota")
	public void addCheckedoutFeatureOverQuotaFromBranch() throws JSONException, IOException{
		//add new feature in production to master
		br.checkoutFeature(branchID, featureID1, sessionToken);
		
		String feature = FileUtils.fileToString(filePath + "feature2.txt", "UTF-8", false);
		JSONObject json = new JSONObject(feature);
		json.put("stage", "PRODUCTION");
		featureID2 = f.addFeatureToBranch(seasonID, "MASTER", json.toString(), "ROOT", sessionToken);
		...
		//add feature to analytics in branch
	...
	@Test (dependsOnMethods="addCheckedoutFeatureOverQuotaFromBranch", description="Add Production feature in master - exceeds the quota")
	public void addFeatureOverQuotaFromMaster() throws JSONException, IOException{

		br.checkoutFeature(branchID, featureID1, sessionToken);
		
		String config = FileUtils.fileToString(filePath + "feature1.txt", "UTF-8", false);
		JSONObject json = new JSONObject(config);
		json.put("name", "F3");
		json.put("stage", "PRODUCTION");
		featureID3 = f.addFeature(seasonID, json.toString(), "ROOT", sessionToken);
		//Assert.assertTrue(featureID3.contains("error"), "Feature3 exceeded quota " + featureID3);
	...
	private String addExperiment(String experimentName) throws IOException, JSONException{
		return baseUtils.addExperiment(m_analyticsUrl, true, true);
	}
