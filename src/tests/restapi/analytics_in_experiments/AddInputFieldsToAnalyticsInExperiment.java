...
public class AddInputFieldsToAnalyticsInExperiment {
	...
	public void deleteFieldFromAnalyticsInSeason1() throws Exception{
		...
		response = an.getGlobalDataCollection(seasonID1, BranchesRestApi.MASTER, "VERBOSE", sessionToken);
		...
		response = an.updateGlobalDataCollection(seasonID1, BranchesRestApi.MASTER, json.toString(), sessionToken);
		Assert.assertFalse(response.contains("error"), "Analytics was not updated" + response);
	...
	public void addFieldToAnalyticsInTwoSeasons() throws Exception{
        ...
        response = an.updateInputFieldToAnalytics(seasonID2, BranchesRestApi.MASTER, inputFields.toString(), sessionToken);
		Assert.assertFalse(response.contains("error"), "Analytics was not updated" + response);
	...
	public void deleteFieldFromAnalyticsInSeason2() throws Exception{
		...
		String response = an.getGlobalDataCollection(seasonID2, BranchesRestApi.MASTER, "VERBOSE", sessionToken);
		Assert.assertFalse(response.contains("error"), "Incorrect globalDataCollection response");
		...
		analyticsDataCollection.remove("inputFieldsForAnalytics");
		analyticsDataCollection.put("inputFieldsForAnalytics", new JSONArray());
		response = an.updateGlobalDataCollection(seasonID2, BranchesRestApi.MASTER, json.toString(), sessionToken);
		Assert.assertFalse(response.contains("error"), "Analytics was not updated" + response);
