 ...
 * - checkout entitlement to branch1 & branch 2
 * - delete variant
 * - delete experiment
 * 
 ...
 */
	...
	public void checkoutEntitlements() throws Exception{
		String dateFormat = purchasesApi.setDateFormat();
		
		//add entitlements to master
		String entitlement1 = FileUtils.fileToString(filePath + "purchases/inAppPurchase1.txt", "UTF-8", false);
		JSONObject jsonE1 = new JSONObject(entitlement1);
		jsonE1.put("stage", "PRODUCTION");
		jsonE1.put("name", "E1");
		entitlementID1 = purchasesApi.addPurchaseItem(seasonID, jsonE1.toString(), "ROOT", sessionToken);
		...
		jsonE2.put("name", "E2");
		entitlementID2 = purchasesApi.addPurchaseItem(seasonID, jsonE2.toString(), "ROOT", sessionToken);
		...
		Assert.assertFalse(response.contains("error"), "entitlement1 was checked out twice");
		
		response = br.checkoutFeature(branchID2, entitlementID2, sessionToken);
		Assert.assertFalse(response.contains("error"), "entitlement2 was checked out twice");
		...
		Assert.assertTrue(getBranchesInRuntime(responseDev.message).getJSONObject(0).getJSONArray("entitlements").getJSONObject(0).getString("name").equals("E1"), "entitlement1 was assigned to incorrect branch");
		Assert.assertTrue(getBranchesInRuntime(responseDev.message).getJSONObject(1).getJSONArray("entitlements").getJSONObject(0).getString("name").equals("E2"), "entitlement2 was assigned to incorrect branch");
	...
	@Test(dependsOnMethods="checkoutEntitlements", description="Change variants order")
